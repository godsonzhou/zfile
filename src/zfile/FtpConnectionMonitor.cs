...
namespace zfile
{
	public class AsyncFtpConnectionMonitor
	{
		...
		public void AddConnection(string connectionName, AsyncFtpClient client)
		{
			if (!_monitoredConnections.ContainsKey(connectionName))
			{
				_monitoredConnections.Add(connectionName, new AsyncFtpMonitorItem
				{
		...
		private void CheckConnectionsStatus(object state)
		{
			...
			foreach (var item in _monitoredConnections)
			{
				...
				AsyncFtpMonitorItem monitorItem = item.Value;
		...
		private bool IsConnectionActive(AsyncFtpClient client)
		{
		...
		/// FTP监控项
		/// </summary>
		private class AsyncFtpMonitorItem
		{
