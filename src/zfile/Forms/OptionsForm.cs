...
namespace zfile
{
	...
	public partial class OptionsForm : Form
	{
		...
		private void InitializeWdxTab(TabPage tabPage)
		{
			// 创建DataGridView显示插件配置
			DataGridView grid = new DataGridView
			{
			...
			// 添加列
			grid.Columns.Add("PluginName", "插件名称");
			grid.Columns.Add("Extension", "规则表达式");
			...
			// 加载现有配置
			if (WdxPlugins._moduleList._configDict != null)
			{
				foreach (var config in WdxPlugins._moduleList._configDict)
				{
			...
			// 添加事件处理
			btnAdd.Click += (s, e) => AddWdxMapping(grid);
			btnDelete.Click += (s, e) => DeleteWdxMapping(grid);
		...
		private void AddWdxMapping(DataGridView grid)
		{
			using var addForm = new AddWdxMappingForm(WdxPlugins._moduleList);
			if (addForm.ShowDialog() == DialogResult.OK)
			{
				grid.Rows.Add(addForm.SelectedPlugin, addForm.Extension);
				// 更新配置
				UpdateWdxConfiguration(grid);
			}
		}

		private void DeleteWdxMapping(DataGridView grid)
		{
			if (grid.SelectedRows.Count > 0)
			{
				grid.Rows.RemoveAt(grid.SelectedRows[0].Index);
		...
		private void UpdateWdxConfiguration(DataGridView grid)
		{
			// 清除现有配置
			if (WdxPlugins._moduleList._configDict != null)
			{
				WdxPlugins._moduleList._configDict.Clear();
		...
		private void Okbutton_Click(object sender, EventArgs e)
		{
			...
			// 保存WDX配置
			WdxPlugins._moduleList.SaveConfiguration();
			// save font
			updateFont();
			UpdateBasicSettings();
			// 应用视图自动切换规则的更改
