...
//						// 找到对应的目标行
//						int targetLine = FindCorrespondingLine(source, target, firstVisibleLine);
						
//						if (targetLine >= 0)
//						{
//							// 计算目标行的像素位置
//							int pos = targetLine * target.Font.Height;
//							pos = Math.Max(0, pos);
							
//							// 设置滚动位置
//							SetScrollPos(target.Handle, 1, pos, true);
//							PostMessage(target.Handle, 0x115, 4 + 0x10000 * pos, 0);
//						}
//					}
//					else
//					{
...
//		// 查找对应行的方法
//		private int FindCorrespondingLine(RichTextBox source, RichTextBox target, int sourceLine)
//		{
//			if (isHexMode) return sourceLine; // 16进制模式直接返回相同行号

//			// 获取源文本框和目标文本框是左侧还是右侧
//			bool isSourceLeft = source == txtLeftContent;
			
//			// 获取当前可见行的实际行号（在原始文件中的行号）
//			string[] sourceLineNumbers = isSourceLeft ? alignedLeftLineNumbersArr : alignedRightLineNumbersArr;
//			string[] targetLineNumbers = isSourceLeft ? alignedRightLineNumbersArr : alignedLeftLineNumbersArr;

//			// 确保行号在有效范围内
//			if (sourceLine < 0 || sourceLine >= sourceLineNumbers.Length)
//				return sourceLine; // 超出范围时直接返回原行号

//			// 获取源行的行号文本
//			string sourceLineNumber = sourceLineNumbers[sourceLine];

//			// 如果是空行（插入或删除的行），查找最近的非空行
//			if (string.IsNullOrEmpty(sourceLineNumber))
//			{
//				// 向下查找最近的非空行
//				for (int i = sourceLine + 1; i < sourceLineNumbers.Length; i++)
//				{
//					if (!string.IsNullOrEmpty(sourceLineNumbers[i]))
//					{
//						sourceLineNumber = sourceLineNumbers[i];
//						sourceLine = i;
//						break;
//					}
//				}
...
//			// 在目标行号数组中查找匹配的行号
//			for (int i = 0; i < targetLineNumbers.Length; i++)
//			{
//				if (targetLineNumbers[i] == sourceLineNumber)
//					return i; // 找到匹配的行号，返回对应的行索引
//			}

//			// 如果没有找到完全匹配的行号，返回原始行号
//			return sourceLine;
//		}
...
//						// 设置文本内容
//						txtLeftContent.Text = string.Join(Environment.NewLine, alignedLeftLines);
//						txtRightContent.Text = string.Join(Environment.NewLine, alignedRightLines);
...
//		{
//			// 在文本模式下，我们已经在 BuildAlignedTextAndLineNumbers 中进行了行对齐
//			// 这里只需要识别差异行并添加到 differences 列表中
...
//			// 确保行号数组已初始化
//			if (alignedLeftLineNumbersArr == null || alignedRightLineNumbersArr == null)
//				return;
...
//			{
//				// 检查是否超出范围
//				bool leftExists = i < leftLines.Length;
//				bool rightExists = i < rightLines.Length;
...
//					differences.Add(new Difference
//					{
//						LeftStart = leftExists ? i : Math.Max(0, i - 1),
//						RightStart = rightExists ? i : Math.Max(0, i - 1),
//						LeftLength = leftExists ? 1 : 0,
//						RightLength = rightExists ? 1 : 0
//					});
//					continue;
...
//				// 获取行号
//				string leftLineNumber = i < alignedLeftLineNumbersArr.Length ? alignedLeftLineNumbersArr[i] : "";
//				string rightLineNumber = i < alignedRightLineNumbersArr.Length ? alignedRightLineNumbersArr[i] : "";
				
//				// 检查是否为空行（插入或删除的行）
//				bool isLeftEmpty = string.IsNullOrEmpty(leftLine) || string.IsNullOrEmpty(leftLineNumber);
//				bool isRightEmpty = string.IsNullOrEmpty(rightLine) || string.IsNullOrEmpty(rightLineNumber);
...
//						LeftStart = i,
//						RightStart = i,
//						LeftLength = isLeftEmpty ? 0 : 1,
//						RightLength = isRightEmpty ? 0 : 1
//					});
//					continue;
//				}
...
//            {
//                isScrolling = true;
//                isSuppressingSyncScroll = true; // 禁用滚动同步，防止导航过程中触发同步
...
//                    {
//                        // 找到左侧差异行对应的右侧行
//                        int alignedRightLine = FindCorrespondingLine(txtLeftContent, txtRightContent, diff.LeftStart);
//                        int alignedPos = alignedRightLine * txtRightContent.Font.Height;
//                        alignedPos = Math.Max(0, alignedPos);
                        
//                        // 设置滚动位置
//                        SetScrollPos(txtRightContent.Handle, 1, alignedPos, true);
//                        PostMessage(txtRightContent.Handle, 0x115, 4 + 0x10000 * alignedPos, 0);
//                    }
//                    else
//                    {
//                        // 16进制模式使用原有逻辑
...
//                // 短暂延迟后恢复滚动同步
//                Task.Delay(100).ContinueWith(_ => { isSuppressingSyncScroll = false; });
//            }
//            finally
//            {
//                isScrolling = false;
